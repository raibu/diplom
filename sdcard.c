...
#define SDIO_INIT_CLK_DIV                  ((uint8_t)0xfc)
#define SDIO_TRANSFER_CLK_DIV              ((uint8_t)0xfc)
...
SD_Error SD_Init(void)
{
  ...
  SDIO->CLKCR = SDIO_CLKCR_HWFC_EN | SDIO_CLKCR_CLKEN | SDIO_TRANSFER_CLK_DIV;
...
SD_Error SD_PowerON(void)
{
  ...
  {
      SDIO->ARG=0;
      SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_GO_IDLE_STATE;
  ...
  SDIO->ARG = SD_CHECK_PATTERN;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SEND_IF_COND;
  ...
  else
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = 0;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_APP_CMD;
  ...
  if (errorstatus == SD_OK)
  {
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
      ...
      // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      // SDIO_SendCommand(&SDIO_CmdInitStructure);
      SDIO->ARG = SD_VOLTAGE_WINDOW_SD | SDType;
      SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SD_APP_OP_COND;
  ...
  else
  {
    ...
    while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
    {
      ...
      // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      // SDIO_SendCommand(&SDIO_CmdInitStructure);
      SDIO->ARG = SD_VOLTAGE_WINDOW_MMC;
      SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SEND_OP_COND;
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = 0;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP | SDIO_ALL_SEND_CID;
      ...
      ||  (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = 0;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SET_REL_ADDR;
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = (uint32_t)(rca << 16);
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP | SDIO_SEND_CSD;
  ...
  else if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    else if (SDIO_BusWide_4b == WideMode)
    {
      ...
      if (SD_OK == errorstatus)
      {
        ...
        // SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        // SDIO_Init(&SDIO_InitStructure);
        SDIO->CLKCR = SDIO_CLKCR_CLKEN | SDIO_CLKCR_WIDBUS_0 | SDIO_TRANSFER_CLK_DIV;
      }
    }
    else
    {
      ...
      if (SD_OK == errorstatus)
      {
        ...
        // SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;
        // SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;
        SDIO->CLKCR = SDIO_CLKCR_CLKEN | SDIO_TRANSFER_CLK_DIV;
      }
    }
  }
...
SD_Error SD_SelectDeselect(uint32_t addr)
{
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = addr;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SEL_DESEL_CARD;
...
SD_Error SD_ReadBlock(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize)
{
  ...
  if ((BlockSize > 0) && (BlockSize <= 2048) && ((BlockSize & (BlockSize - 1)) == 0))
  {
    ...
    // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = (uint32_t) BlockSize;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SET_BLOCKLEN;
  ...
  SDIO->DTIMER = SD_DATATIMEOUT;
  SDIO->DLEN = BlockSize;
  SDIO->DCTRL = ((uint32_t) power << 4) | SDIO_DCTRL_DTDIR | SDIO_DCTRL_DTEN;
  ...
  // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = (uint32_t)addr;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_READ_SINGLE_BLOCK;
  ...
  /* In case of single block transfer, no need of stop transfer at all.*/
    ...
    else if ((SDIO->STA & SDIO_STA_STBITERR) != 0)
    {
...
SD_Error SD_ReadMultiBlocks(uint32_t addr, uint32_t *readbuff, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  if (NumberOfBlocks > 1)
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = (uint32_t)addr;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_READ_MULT_BLOCK;
    ...
    if (errorstatus != SD_OK)
    {
      ...
      while (!(SDIO->STA &(SDIO_STA_RXOVERR | SDIO_STA_DCRCFAIL | SDIO_STA_DATAEND | SDIO_STA_DTIMEOUT | SDIO_STA_STBITERR)))
      {
        if ((SDIO->STA & SDIO_STA_RXFIFOHF) != RESET)
        {
      ...
      if ((SDIO->STA & SDIO_STA_DATAEND) != 0)
      {
        ...
        if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType))
        {
          ...
          // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
          SDIO->ARG = 0;
          SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_STOP_TRANSMISSION;
...
SD_Error SD_WriteBlock(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize)
{
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = (uint32_t) (RCA << 16);
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SEND_STATUS;
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = addr;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_WRITE_SINGLE_BLOCK;
  ...
  SDIO->DTIMER = SD_DATATIMEOUT;
  SDIO->DLEN = BlockSize;
  SDIO->DCTRL = ((uint32_t) power << 4) | SDIO_DCTRL_DTEN;
  /* In case of single data block transfer no need of stop command at all */
    while (!(SDIO->STA & (SDIO_STA_DBCKEND | SDIO_STA_TXUNDERR | SDIO_STA_DCRCFAIL | SDIO_STA_DTIMEOUT | SDIO_STA_STBITERR)))
    {
...
SD_Error SD_WriteMultiBlocks(uint32_t addr, uint32_t *writebuff, uint16_t BlockSize, uint32_t NumberOfBlocks)
{
  ...
  if (NumberOfBlocks > 1)
  {
    ...
    if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
    {
      ...
      // SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
      // SDIO_SendCommand(&SDIO_CmdInitStructure);
      SDIO->ARG = (uint32_t) NumberOfBlocks;
      SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SET_BLOCK_COUNT;
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = (uint32_t) addr;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_WRITE_MULT_BLOCK;
    ...
    SDIO->DCTRL = ((uint32_t) power << 4) | SDIO_DCTRL_DTEN;
      ...
      while (!(SDIO->STA & (SDIO_STA_TXUNDERR | SDIO_STA_DCRCFAIL | SDIO_STA_DATAEND | SDIO_STA_DTIMEOUT | SDIO_STA_STBITERR)))
      {
        if ((SDIO->STA & SDIO_STA_TXFIFOHE) != 0)
        {
...
{
  if (SDIO->STA & (SDIO_STA_TXACT | SDIO_STA_RXACT))
  {
...
SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr)
{
  ...
  if ((SDIO_STD_CAPACITY_SD_CARD_V1_1 == CardType) || (SDIO_STD_CAPACITY_SD_CARD_V2_0 == CardType) || (SDIO_HIGH_CAPACITY_SD_CARD == CardType))
  {
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = startaddr;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SD_ERASE_GRP_START;
    ...
    // SDIO_SendCommand(&SDIO_CmdInitStructure);
    SDIO->ARG = endaddr;
    SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SD_ERASE_GRP_END;
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = 0;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_ERASE;
...
SD_Error SD_SendSDStatus(uint32_t *psdstatus)
{
  ...
  SDIO->DTIMER = SD_DATATIMEOUT;
  SDIO->DLEN = 64;
  SDIO->DCTRL = ((uint32_t) 6 << 4) | SDIO_DCTRL_DTDIR | SDIO_DCTRL_DTEN;
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = 0;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SD_APP_STAUS;
...
static SD_Error SDEnWideBus(FunctionalState NewState)
{
  ...
  if (NewState == ENABLE)
  {
    ...
    if ((scr[1] & SD_WIDE_BUS_SUPPORT) != SD_ALLZERO)
    {
      ...
      // SDIO_SendCommand(&SDIO_CmdInitStructure);
      SDIO->ARG = 2;
      SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_APP_SD_SET_BUSWIDTH;
...
static SD_Error FindSCR(uint16_t rca, uint32_t *pscr)
{
  ...
  SDIO->DTIMER = SD_DATATIMEOUT;
  SDIO->DLEN = 8;
  SDIO->DCTRL = ((uint32_t) 3 << 4) | SDIO_DCTRL_DTDIR | SDIO_DCTRL_DTEN;
  ...
  // SDIO_SendCommand(&SDIO_CmdInitStructure);
  SDIO->ARG = 0;
  SDIO->CMD = SDIO_CMD_CPSMEN | SDIO_CMD_WAITRESP_0 | SDIO_SD_APP_SEND_SCR; 
...
static void GPIO_Configuration(void)
{
  ...
  /* Configure PC8 and PC12 as D0 and CLK */
  ...
  GPIOC->MODER |= GPIO_MODER_MODER8_1 | 
                  ...
                  GPIO_MODER_MODER12_1; //mode AFPP
  GPIOC->OSPEEDR |= GPIO_OSPEEDR_OSPEED8 | 
                    GPIO_OSPEEDR_OSPEED9 |
                    GPIO_OSPEEDR_OSPEED10 | GPIO_OSPEEDR_OSPEED11 |
                    GPIO_OSPEEDR_OSPEED12; //high speed;
  GPIOC->AFR[1] |= (12<<GPIO_AFRH_AFSEL8_Pos) | 
                    (12<<GPIO_AFRH_AFSEL9_Pos) |
                    (12<<GPIO_AFRH_AFSEL10_Pos) | (12<<GPIO_AFRH_AFSEL11_Pos) |
                    (12<<GPIO_AFRH_AFSEL12_Pos) ; //map AF12 to pins;


  /* Configure PD2 as CMD line */
...
uint32_t SDIO_GetResponse(uint32_t SDIO_RESP){
  return ((volatile uint32_t*)(SDIO->RESP1))[SDIO_RESP];
}
